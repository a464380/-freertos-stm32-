...
int errorNum = 0;	//解锁错误次数

extern uint8_t ReadData[1];
extern uint8_t WriteData[1];
...
//		ID_NUM = 1;
	  ...
	  W25Q64_ReadData(0x000000, ReadData, 1);		//读取id
	  if(WriteData[0] >= 200)						//指纹上限
	  {
		  broadcast_sound(2, 1000);
		  return;
	  }
	  WriteData[0] = (ReadData[0] + 1);				//将读取到的id+1
	  W25Q64_SectorErase(0x000000);					//清空扇区
	  W25Q64_PageProgram(0x000000, WriteData, 1);	//写入id
	  
      ensure = PS_StoreChar(CharBuffer2, WriteData[0]); //储存模板
      if(ensure == 0x00)
      {
				printf("录入指纹成功\r\n");
				printf("录入指纹id为:%d\r\n", WriteData[0]);
				broadcast_sound(1, 1000);
				addfingerstate = 0;
        HAL_Delay(1000);
        return ;
      }
...
void press_FR(void)
{
  ...
  uint8_t ensure;
  char str[20];
  while(!addfingerstate && lockState == 1 && !removefingerstate)		//如果不在添加指纹状态同时在上锁状态 则一直扫描
  {
    //key_num = KEY_Scan(0);
    ensure = PS_GetImage();
    if(ensure == 0x00) //获取图像成功
    {
      ...
      if(ensure == 0x00) //生成特征成功
      {
        ...
        else
        {
					...
					errorNum++;
					//HAL_Delay(500);
					if(errorNum >= 5 && errorNum < 7)		//如果错误次数大于5
					{
						//提醒
						broadcast_sound(12, 1000);
					}else if(errorNum >= 7 && errorNum < 10)
					{
						//提醒报警
...
void Del_FR_Lib(void)
{
  ...
  if(ensure == 0)
  {
	    ...
	    W25Q64_PageProgram(0x000000, WriteData, 1);
		broadcast_sound(5, 1000);
		printf("清空指纹库成功\r\n");
  }
  else
     ShowErrMessage(ensure);
